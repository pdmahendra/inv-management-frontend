...
function EditAstarItem({ onEditClick, row }) {
  ...
  React.useEffect(() => {
    if (row) {
      setData({
        item_name: row.original?.extra_fields[0]?.item_name,
        color: row.original?.extra_fields[1]?.color,
